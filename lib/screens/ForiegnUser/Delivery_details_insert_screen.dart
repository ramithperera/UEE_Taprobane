...
class DeliveryDetailsInsert extends StatefulWidget {
  DeliveryDetailsInsert({Key? key}) : super(key: key);

  @override
  State<DeliveryDetailsInsert> createState() => _DeliveryDetailsInsertState();
}

class _DeliveryDetailsInsertState extends State<DeliveryDetailsInsert> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Add New Delivery Address Details"),
      ),
      bottomNavigationBar:Padding(
            padding: EdgeInsets.all(20),
            child: ElevatedButton(
              onPressed: () {
                insertDeliveryAddressDetails();
              },
              child: Container(
                height: 50,
  ...
  void insertDeliveryAddressDetails() async {
    ...
    else
    {
      ...
      String? response = await userAddressInsert(context, body);
      if (response != null) {
        showToastMessage('Add New Address Success!');
        SharedPreferences prefs = await SharedPreferences.getInstance();
